---
title: R code and output for Arctic time series analysis
author: Spencer Blackwood and Vyacheslav Lyubchich
date: today
date-format: iso
format:
  html:
...
This report uses R [@R-base] and R packages 
`dplyr` [@R-dplyr],
`tidyr` [@R-tidyr],
`readxl` [@R-readxl],
`ggplot2` [@R-ggplot2],
...
`htmltools` [@R-htmltools],
`mapview` [@R-mapview],
`webshot` [@R-webshot],
`car` [@R-car],
`sf` [@R-sf],
`marmap` [@R-marmap],
`rnaturalearth` [@R-rnaturalearth],
`imputeTS` [@R-imputeTS],
`fpc` [@R-fpc],
`cluster` [@R-cluster],
`zoo` [@R-zoo],
...
geo_labels <- data.frame(
  label = c("Bering Sea", "Alaska (U.S.A.)", "Russia", "Chukchi Sea", "Chirikov Basin", "Bering Strait", "St. Lawrence Island"),
  lon = c(-169, -163, -176, -173, -169, -168.5, -170.25),
  lat = c(62, 65.5, 66, 68.5, 64.25, 66, 63.5)
)
...
map_plot <- ggplot() +
  ...
  # add minor ticks at each 1 degree
  scale_x_continuous(breaks = seq(min_lon, max_lon, by = 2)) +
  scale_y_continuous(breaks = seq(min_lat, max_lat, by = 1))
...
# Save the map
png(file = "output/map0.png", width = 7, height = 6, res = 600, units = "in")
print(map_plot)
dev.off()
```
...
Apply and compare clustering methods for varying number of clusters $k$. Since the true data grouping is unknown, we use the cluster validation indices, average silhouette width and modified Dunn index, to compare different options. Both of these indices evaluate compactness and separation of clusters (higher values indicate better-defined clusters).
...
for (i in 1:length(CLresults)) {
  for (cltype in CT) {
    ...
    if (grepl('dbs', cltype)) {
      mapshot(mapx, file = paste0('./output/AR/ARmap', vars2study[i], 'DBS.png'))
    }
    else {
      mapshot(mapx, file = paste0('./output/AR/ARmap', vars2study[i], '.png'))
    }
    ...
    for (j in unique(coordstmp$Cluster)) {
      ...
      if (grepl('dbs', cltype)) {
        png(file = paste0('./output/AR/ARplotD', i, '_', j, '.png'),
            width = 960, height = 960, pointsize = 24)
      }
      else {
        png(file = paste0('./output/AR/ARplot', i, '_', j, '.png'),
            width = 960, height = 960, pointsize = 24)
      }
      par(mar = c(5.5, 6.2, 2, 1.7) + 0.1)
      plot(2010:2019, unlist(data_tmp[,cluster_tmp[1]]), type = 'b', col = color_tmp, lwd = 4,
           xlab = 'Year', ylab = '', ylim = c(minval, maxval), cex.axis = 2,
           cex.lab = 2, las = 1)
      title(ylab = axnames[i], line = 3.7, cex.lab = 2)
...
# Build the dissimilarity matrices
...
Apply and compare clustering methods for varying number of clusters $k$. Since the true data grouping is unknown, we use the cluster validation indices — average silhouette width, the C-H index, and the WB ratio — to compare different options. These indices evaluate compactness and separation of clusters. For average silhouette width and the C-H index, higher values indicate better-defined clusters, whereas lower values indicate the same for the WB ratio. We produce both a globular clustering as well as a density-based clustering for comparison.
...
for (i in 1:length(results_dtw)) {
  for (cltype in CT) {
    ...
    mapx <- leaflet(coordstmp) %>%
      ...
      addCircleMarkers(radius = 6, stroke = TRUE, weight = 1, color = '#000', fillColor = coordstmp$color, fillOpacity = 0.9)
    if (grepl('dbs', cltype)) {
      mapshot(mapx, file = paste0('./output/DTW/map', vars2study[i], 'DBS.png'))
    }
    else {
      mapshot(mapx, file = paste0('./output/DTW/map', vars2study[i], '.png'))
    }
    ...
    for (j in unique(coordstmp$Cluster)) {
      ...
      maxval <- max(data_tmp, na.rm = T)
      if (grepl('dbs', cltype)) {
        png(file = paste0('./output/DTW/plotD', i, '_', j, '.png'),
            width = 960, height = 960, pointsize = 24)
      }
      else {
        png(file = paste0('./output/DTW/plot', i, '_', j, '.png'),
            width = 960, height = 960, pointsize = 24)
      }
      par(mar = c(5.5, 6.2, 2, 1.7) + 0.1)
      plot(2010:2019, unlist(data_tmp[,cluster_tmp[1]]), type = 'b', col = color_tmp, lwd = 4,
...
for (i in vars2study) {
  mapx <- leaflet(data_powerplot) %>%
      addTiles() %>%
      addCircleMarkers(radius = ~get(i)*16, stroke = TRUE, weight = 1, color = '#000', fillColor = '#de5012', fillOpacity = 0.9)
  mapshot(mapx, file = paste0('./output/power/powermap', i, '.png'))
}
